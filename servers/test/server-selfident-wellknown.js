...
TD.commonCase('ask server for selfident via well-known URL', function(T) {
  var client = T.actor('testClient', 'C');
  var server = T.actor('testServer', 'S', {roles: ['auth', 'signup']});

  T.check('verify', client._eRawClient,
           "gets the server's selfIdent", function() {

    client.assertInsecureFetchedSelfIdent(server);
  });
});
